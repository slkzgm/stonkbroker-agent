...
server.registerTool("broker_status", {
    description: "Inspect a StonkBroker NFT, verify its ERC-6551 binding, and list canonical Robinhood stock-token balances in its token-bound wallet.",
    inputSchema: {
        tokenId: z.number().int().positive().optional().describe("StonkBroker token ID"),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
}, async ({ tokenId }) => toolResult(async () => {
...
server.registerTool("list_stock_tokens", {
    description: "List canonical stock tokens from Robinhood's live asset registry. Use a symbol or contract substring to filter results.",
    inputSchema: {
        query: z.string().trim().optional(),
        limit: z.number().int().min(1).max(100).default(25),
    },
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
}, async ({ query, limit }) => toolResult(async () => {
...
server.registerTool("quote_stock_trade", {
    description: "Request a time-limited Uniswap quote for an exact-input trade between canonical Robinhood stock tokens held by a StonkBroker TBA. This never signs or submits a transaction.",
    inputSchema: {
...
}, async ({ tokenId, tokenIn, tokenOut, amount, slippagePercent }) => toolResult(async () => {
    ...
    return {
        ...
        nextStep: "Call execute_stock_trade with this quoteId and confirmation=EXECUTE before the quote expires.",
    };
}));
server.registerTool("execute_stock_trade", {
    description: "Execute a previously quoted trade from the StonkBroker ERC-6551 wallet, wait for confirmation, then automatically post the actual balance deltas and transaction link to X.",
    inputSchema: {
        quoteId: z.uuid(),
...
server.registerTool("retry_x_posts", {
    description: "Retry every queued X post for confirmed trades. Confirmed trades are durably queued before the first posting attempt.",
    inputSchema: {},
...
async function toolResult(action) {
    try {
        return {
            content: [{ type: "text", text: json(await action()) }],
        };
    }
    catch (error) {
        return {
...
const retryTimer = setInterval(() => {
    if (retrying || !config.xUserAccessToken || config.xDryRun)
        return;
    retrying = true;
    void trader
        .flushPendingPosts()
