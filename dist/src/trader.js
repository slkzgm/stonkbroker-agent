...
import { createWalletClient, decodeAbiParameters, decodeFunctionData, encodeFunctionData, formatUnits, getAddress, http, parseAbiParameters, parseUnits, } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { erc20Abi, stonkBrokerAccountAbi, uniswapSwapProxyAbi } from "./abis.js";
import { BLOCKSCOUT_URL, CHAIN_ID, UNISWAP_SWAP_PROXY, UNISWAP_UNIVERSAL_ROUTER, robinhoodChain, } from "./constants.js";
...
const v3ExactInputParameters = parseAbiParameters("address recipient, uint256 amountIn, uint256 amountOutMin, bytes path, bool payerIsUser, uint256[] minHopPriceX36");
const v2ExactInputParameters = parseAbiParameters("address recipient, uint256 amountIn, uint256 amountOutMin, address[] path, bool payerIsUser, uint256[] minHopPriceX36");
const paymentParameters = parseAbiParameters("address token, address recipient, uint256 amount");
const commandsAndInputsParameters = parseAbiParameters("bytes commands, bytes[] inputs");
const v4SettleParameters = parseAbiParameters("address currency, uint256 amount, bool payerIsUser");
const v4TakeParameters = parseAbiParameters("address currency, address recipient, uint256 amount");
export class StonkTrader {
    ...
    outbox;
    publisher;
    constructor(config, broker, uniswap, outbox, publisher, transactionExecutor) {
        this.config = config;
        this.broker = broker;
    ...
    async executeTrade(quoteId) {
        ...
        const [swap, inputBalanceBefore, outputBalanceBefore, currentAllowance] = await Promise.all([
            ...
            this.broker.tokenAllowance(quote.wallet, quote.tokenIn.address, UNISWAP_SWAP_PROXY),
        ]);
        validateSwapTransaction(swap, quote.wallet, quote.tokenIn.address, quote.tokenOut.address, quote.amountIn);
        if (Date.now() > quote.expiresAt) {
        ...
        for (const amount of requiredApprovalAmounts(currentAllowance, quote.amountIn)) {
            const transaction = approvalTransaction(quote.wallet, quote.tokenIn.address, UNISWAP_SWAP_PROXY, amount);
            validateApprovalTransaction(transaction, quote.wallet, quote.tokenIn.address, UNISWAP_SWAP_PROXY, amount);
            approvalTxHashes.push(await this.executeTbaCall(transaction, quote.wallet, account.address));
        }
        const finalAllowance = await this.broker.tokenAllowance(quote.wallet, quote.tokenIn.address, UNISWAP_SWAP_PROXY);
        if (finalAllowance !== quote.amountIn) {
            throw new Error(`Input-token allowance is ${finalAllowance}, expected exactly ${quote.amountIn}`);
        }
    ...
    async executeTbaCall(transaction, wallet, owner) {
...
export function validateSwapTransaction(transaction, wallet, tokenIn, tokenOut, exactAmountIn, nowSeconds = BigInt(Math.floor(Date.now() / 1_000))) {
    ...
    let decoded;
    try {
        decoded = decodeFunctionData({ abi: uniswapSwapProxyAbi, data: transaction.data });
    }
    catch {
...
function validateRouterPlan(commands, inputs, wallet, tokenOut) {
...
function validateRouterCommands(commands, inputs, wallet, tokenOut, state) {
    ...
    for (let index = 0; index < commandBytes.length; index += 1) {
        ...
        try {
            if (command === 0x00) {
                const [recipient, , , path, payerIsUser] = decodeAbiParameters(v3ExactInputParameters, input);
                assertSafeRecipient(recipient, wallet);
                if (payerIsUser)
                    throw new Error("V3 swap asks the proxy to pay through Permit2");
                if (getAddress(v3OutputToken(path)) === getAddress(tokenOut)) {
                    markOutputRecipient(recipient, wallet, state);
                }
            }
            else if (command === 0x08) {
                const [recipient, , , path, payerIsUser] = decodeAbiParameters(v2ExactInputParameters, input);
                assertSafeRecipient(recipient, wallet);
                if (payerIsUser)
                    throw new Error("V2 swap asks the proxy to pay through Permit2");
                const routeOutput = path.at(-1);
                if (!routeOutput)
                    throw new Error("V2 swap path is empty");
...
function validateV4Plan(input, wallet, tokenOut, state) {
    ...
    for (let index = 0; index < actionBytes.length; index += 1) {
        ...
        if (action === 0x06 || action === 0x07) {
            ...
            if (parametersForAction === "0x")
                throw new Error("Uniswap V4 swap parameters are empty");
        }
        else if (action === 0x0b) {
            const [, , payerIsUser] = decodeAbiParameters(v4SettleParameters, parametersForAction);
            if (payerIsUser)
                throw new Error("V4 settlement asks the proxy to pay through Permit2");
        }
        else if (action === 0x0c) {
            ...
            const [currency, recipient, amount] = decodeAbiParameters(v4TakeParameters, parametersForAction);
            assertSafeRecipient(recipient, wallet);
...
function markOutputRecipient(recipient, wallet, state) {
    if (isWallet(recipient, wallet))
        state.delivered = true;
    else
        state.routerHoldsOutput = true;
}
function assertSafeRecipient(recipient, wallet) {
...
function isWallet(recipient, wallet) {
    return getAddress(recipient) === getAddress(wallet);
}
function v3OutputToken(path) {
...
function hexBytes(value) {
    ...
    if (raw.length === 0 || raw.length % 2 !== 0)
        throw new Error("Command bytes are malformed");
    return raw.match(/.{2}/g).map((byte) => Number.parseInt(byte, 16));
}
export function validateApprovalTransaction(transaction, wallet, tokenIn, swapTarget, exactApprovalAmount) {
    if (transaction.chainId !== CHAIN_ID)
...
export function requiredApprovalAmounts(currentAllowance, exactAmountIn) {
    if (currentAllowance === exactAmountIn)
        return [];
    return currentAllowance === 0n ? [exactAmountIn] : [0n, exactAmountIn];
}
function approvalTransaction(wallet, token, spender, amount) {
    return {
        from: wallet,
        to: token,
        chainId: CHAIN_ID,
