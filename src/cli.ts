...
const trader = new StonkTrader(
  config,
  broker,
  config.uniswapApiKey ? new UniswapClient(config.uniswapApiKey) : undefined,
);
...
switch (command) {
  ...
  case "tokens": {
    ...
    const assets = (await broker.assets.list()).filter(
      (asset) =>
        !query ||
...
function tokenIdFlag(flags: Map<string, string>): bigint {
